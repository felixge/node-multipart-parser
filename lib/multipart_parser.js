  ...
  for (var i = 0; i < buffer.length; i++) {
    ...
    switch (this._state) {
      ...
      case 'HEADER_VALUE':
        ...
        break;
      case 'PART_BODY':
        if (this._counter === this._boundary.length - 4) {
          if (byte === CR) {
            this._lastBoundary = false;
            this._counter     += 2;
          } else if (byte === DASH) {
            this._lastBoundary = true;
          }
        }

        if (byte === this._boundary[this._counter]) {
          if (this._counter === 0) {
            this._part.emit('data', buffer.slice(this._marker, i));
          } else if (this._counter === this._boundary.length - 1) {
            if (this._lastBoundary) {
              this.emit('end');
              this._state = 'EPILOGUE';
            } else {
              //this._part.emit('data', buffer.slice(this._marker, i + 1));
              this._state   = 'HEADER_FIELD';
              this._counter = 0;
              this._marker  = 0;
              this._part    = null;
          ...
          this._marker  = i;
          this._counter = (byte === this._boundary[0])
            ? 1
            : 0;
        }
